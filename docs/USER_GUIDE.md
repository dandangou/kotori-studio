...
这份手册按第一次使用的顺序演示。截图来自独立的 **32 秒合成演示项目**；测试画面、示例字幕和手动候选只用于说明按钮，不是直播素材，也不是 ASR、翻译或自动选片的效果评测。点击图片可放大查看。
...
![工作台总览：项目、处理流程、播放器、候选、波形和字幕](images/overview.png)
...
![导入窗口：本地路径与 YouTube 画质选择](images/import.png)
...
![模型与设置：安装状态、下载按钮和下载来源](images/models.png)
...
![项目资料：填写标题、来源链接和术语表](images/glossary.png)
...
![日语识别：选择模型与当前选区或整个视频](images/asr.png)
...
![中文初译：本地模型、范围与覆盖设置](images/translation.png)
...
![字幕时间轴：15 秒视窗、选中字幕和中日文编辑栏](images/timing.png)
...
![整体偏移：选择作用范围并输入正负秒数](images/timing-offset.png)
...
![角色设置：角色名、颜色选择器和预览](images/speakers.png)
...
![拼接清单：原片入出点、成片累计时间、段落顺序与保存入口](images/assembly.png)
...
![我的切片：多个命名作品与新建或更新入口](images/clips.png)
...
![批量导出：每个切片分别生成文件，选择格式、分辨率与字幕语言](images/export.png)
