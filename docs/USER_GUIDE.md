...
字幕工具栏的「假名注音」可开关校对用读音。开启后，日文编辑框上方显示带平假名的参考行；修改日文后自动更新，只处理当前页。注音由本地 [Janome 词典](https://janome.mocobeta.dev/en/)生成，不调用外部 API，也不写入原文、译文、SRT、ASS 或烧录视频。人名和多音字仍需听原音确认；关闭后恢复普通编辑视图。
